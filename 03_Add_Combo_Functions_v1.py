""" Component 2 -- Add Combo Functions version 1
Adding a string boundary function so unnecessary code
isn't repeat and so the user input is not too long
Written by Katelyn
...
def char_boundary(upper, lower, question, box_name):
    while True:
        ...
        if len(asking) > upper:
            easygui.msgbox(f"Wrong Input. Please enter less than {upper} letters")

        # Lower string boundary
        elif len(asking) < (lower + 1):
            easygui.msgbox(f"Wrong Input. Please more than {lower} letters")
